...
function errorHandling() {
  axios({
    method:'get',
    url:'https://jsonplaceholder.typicode.com/todospppppppppppppppppppppppppp'
  } ).then((res)=>showOutput(res))
      .catch((err)=>{
        if(err.response){ 
...
function cancelToken() {
 ...
 cancelToken: source.token}
    ...
    if(true)source.cancel('for unknown reasons')
}

// INTERCEPTING REQUESTS & RESPONSES
axios.interceptors.request.use(config=>{
  console.log(config.method +'req sent to'+config.url ,'at'+new Date().getTime())
  return config
